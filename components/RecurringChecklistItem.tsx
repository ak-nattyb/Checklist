...
export function RecurringChecklistItem({ text, id }: ButtonProps) {
  ...
  const isChecked = useChecklistStore((state) => {
    ...
    return entry?.kind === "recurringitem" ? entry.isChecked : false;
  });

  function check() {
    if (!isChecked) useChecklistStore.getState().toggleItem(id);
  }
  ...
  return (
    ...
    >
      {justifyText === "Left" ? (
        ...
        >
          <MaterialIcons
            name={isChecked ? "access-time" : "check-box-outline-blank"}
            size={
              displayMode === "Lg"
                ? n(36) //100%
                : displayMode === "Md"
